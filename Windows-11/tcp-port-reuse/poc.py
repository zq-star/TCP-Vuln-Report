...
def main():
    ...
    if new_port == syn_packet[0]:
        ...
        syn1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="S", seq=rand_seq, ack=syn_packet[2] + 1)
        syn_ack1 = sr1(syn1)

        rst1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="R", seq=syn_ack1.ack)
        send(rst1)
        #time.sleep(10)
        client_socket.send("connect\n")
        #send(syn_ack)
