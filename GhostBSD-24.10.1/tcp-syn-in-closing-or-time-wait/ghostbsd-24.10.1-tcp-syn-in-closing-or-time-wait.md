...
2. In test machine - virtual machine 1:
   ...
   * Run `sudo python poc.py`. The [poc.py](https://github.com/zq-star/TCP-Vuln-Report/blob/master/GhostBSD-24.10.1/tcp-syn-in-closing-or-time-wait/poc.py) sends packets of specified types in order: CONNECT, SYN+ACK(V, V), CLOSE, ACCEPT, FIN+ACK(V, V), SYN(INV, 0).
