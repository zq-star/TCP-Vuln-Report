...
#define input_buffer_size 1025000
#define output_buffer_size 1025000
...
int process_input() {
	...
	int result = recv(learner_conn_sd, read_buffer, sizeof(read_buffer), 0);
#elif defined (__gnu_linux__) || defined (__FreeBSD__)
	printf("read_buffer non1111111111111111111");
	int result = read(learner_conn_sd, read_buffer, sizeof(read_buffer));
	printf("read_buffer non222222222222222222222");
#endif
	if (result <= 0) { // either -1 for an error, or 0 if connection is closed properly
		return -1;
	}
