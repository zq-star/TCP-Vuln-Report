...
def main():
    ...
    if new_port == syn_packet[0]:
        ...
        for i in range(n):
            ...
            rst_syn_unvalid = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="RS", seq = seq)
            send(rst_syn_unvalid)
        # rst_syn_unvalid = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="RS", seq=ack1.ack + 60000)
        # send(rst_syn_unvalid)
