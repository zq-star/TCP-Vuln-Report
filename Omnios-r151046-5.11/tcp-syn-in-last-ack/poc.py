...
SERVER_ADDR = '192.168.56.109'  # SUT IP地址:115-Windows, 114-ubuntu(host-own)
...
def main():
    ...
    rand_ack = random.randint(65536, 4294967295)
    ...
    '''
        ...
        send(push_ack2)
    '''
    syn1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="S", seq=rand_seq)
    syn_ack1 = sr1(syn1)
    ack1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="A", seq=syn_ack1.ack, ack=syn_ack1.seq+1)
    send(ack1)
    fin_ack1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="FA", seq=ack1.seq, ack=ack1.ack)
    ack2 = sr1(fin_ack1)
    client_socket.send("accept\n")
    time.sleep(0.6)
    client_socket.send("closeconnection\n")

    syn_ack2 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="SA", seq=ack2.ack, ack=ack2.seq + 1)
    syn2 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="S", seq=ack2.ack)
    syn2_u = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="S", seq=ack2.ack+SERVER_WINDOWS)
    rst1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="R", seq=ack2.ack+SERVER_WINDOWS)
    psh_ack1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="PA", seq=ack2.ack, ack=ack2.seq)/Raw(b'\x01')
    psh1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="P", seq=ack2.ack+SERVER_WINDOWS, ack=rand_ack_unvalid)/Raw(b'\x01')    
    #send(syn_ack2)
    #send(syn2)
    send(syn2_u)
    #send(rst1)
