...
def main():
    ...
    # 连接到服务器
    connect_to_server()
    new_port = sniff_for_new_port(SERVER_CMD_PORT, 0.6)
    print (new_port)
    ...
    if new_port == syn_packet[0]:
        ...
        for i in range(n):
            ...
            rst_push_unvalid = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="RP", seq = seq)
            send(rst_push_unvalid)
        # rst_push_unvalid = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="RP", seq=ack1.ack + 60000)
        # send(rst_push_unvalid)
