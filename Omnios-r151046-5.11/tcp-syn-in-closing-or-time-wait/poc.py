...
def callback(hdr,data):
    ...
    if isinstance(l2,IP):
        ...
        '''
        if l3.get_FIN() and tcp_dst_port == LEARNER_PORT:
            #process_cmd_pa_packet()
            last_fin_responses[tcp_dst_port] = [tcp_src_port, tcp_dst_port, tcp_syn, tcp_ack]
        '''   
...
def sniff_for_syn_packet(learner_port, wait_time):
    ...
    return syn_packet 
'''
def get_fin_packet(learner_port):
    last_fin_response = last_fin_responses.get(learner_port)
    return last_fin_response  
     
def sniff_for_fin_packet(learner_port, wait_time):
    div = wait_time/10
    #print "sniffing for response ", wait_time
    for i in range(0,9):
        #print "waiting... ", div
        time.sleep(div)
        fin_packet = get_fin_packet(learner_port)     
        #self._received.wait(timeout=wait_time)
        #response = self.getLastCmdResponse(serverPort, senderPort)    
        #self._received.clear()
    return fin_packet 
...
def main():
    ...
    syn_ack1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="SA", seq=rand_seq, ack=syn_packet[2] + 1)
    ack1 = sr1(syn_ack1)
    client_socket.send("close\n")
    fin_ack1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="FA", seq=ack1.ack, ack=ack1.seq + 1)
    ack2 = sr1(fin_ack1)
    syn1_u = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="S", seq=ack2.ack+SERVER_WINDOWS)
