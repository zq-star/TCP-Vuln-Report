...
def main():
    ...
    syn2 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="S", seq=syn_ack1.ack)
    syn2_u = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="S", seq=syn_ack1.ack + SERVER_WINDOWS)
    syn_ack2 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="SA", seq=syn_ack1.ack, ack=syn_ack1.seq+1)
    fin1_u = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="F", seq=syn_ack1.ack + SERVER_WINDOWS, ack=rand_ack_unvalid)
    psh1_u = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="P", seq=syn_ack1.ack + SERVER_WINDOWS, ack=rand_ack_unvalid)/Raw(b'\x01')
    #send(syn2)
    send(syn2_u)
    #send(syn_ack2)
    #send(fin1_u)
