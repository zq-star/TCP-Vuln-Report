...
def main():
    ...
    if new_port == syn_packet[0]:
        ...
        for i in range(n):
            ...
            rst_ack_unvalid = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="RA", seq = seq)
            send(rst_ack_unvalid)
        # rst_ack_unvalid = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="RA", seq=ack1.ack + 60000)
        # send(rst_ack_unvalid)
