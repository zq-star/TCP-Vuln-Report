...
def main():
    ...
    if new_port == syn_packet[0]:
        ...
        for i in range(n):
            ...
            syn_fin_unvalid = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="SF", seq = seq)
            send(syn_fin_unvalid)
        # syn_fin_unvalid = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="SF", seq=ack1.ack + 60000)
        # send(syn_fin_unvalid)
