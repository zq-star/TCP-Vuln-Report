...
def main():
    ...
    if new_port == syn_packet[0]:
        ...
        for i in range(n):
            ...
            syn_push_unvalid = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="SP", seq = seq)
            send(syn_push_unvalid)
        # syn_push_unvalid = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=syn_packet[1], dport=syn_packet[0], flags="SP", seq=ack1.ack + 60000)
        # send(syn_push_unvalid)
