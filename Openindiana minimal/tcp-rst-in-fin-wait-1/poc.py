...
def main():
    ...
    '''
        ...
        send(push_ack2)
    '''
    syn1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="S", seq=rand_seq, ack=rand_ack)
    syn_ack1 = sr1(syn1)
    ...
    syn_ack2 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="SA", seq=ack1.seq, ack=ack1.ack)
    syn2 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="S", seq=ack1.seq)
    syn2_u = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="S", seq=ack1.seq+SERVER_WINDOWS)
    rst1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="R", seq=ack1.seq+SERVER_WINDOWS)
    psh1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="P", seq=ack1.seq+SERVER_WINDOWS, ack=rand_ack_unvalid)/Raw(b'\x01')  
    syn4_u = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="S", seq=rand_seq_unvalid)  
    #send(syn_ack2)
    #send(syn2)
    #send(syn2_u)
    send(rst1)
