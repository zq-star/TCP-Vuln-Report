...
def main():
    ...
    send(ack1)
    rst1 = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="R", seq=ack1.seq, ack=ack1.ack)
    send(rst1)

    syn2_u = ScapyIP(dst=SERVER_ADDR) / ScapyTCP(sport=LEARNER_PORT, dport=new_port, flags="S", seq=rand_seq_unvalid)
    send(syn2_u)
